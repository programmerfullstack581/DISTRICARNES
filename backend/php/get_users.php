...
try {
  ...
  $tablesStmt = $conexion->query("SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'");
  ...
  if (!in_array('usuario', $tables)) {
    ...
    ");
    error_log("Tabla usuario creada");
  }
  ...
  if (!$adminExists) {
    ...
    error_log("Admin fijo insertado");
  }
